...
def share_card():
    ...
    print(f"[CARD DEBUG] Requested share_card for user {user.get('id')} / {ref_id} with type={card_type}, arguments={dict(request.args)}", flush=True)
    ...
    try:
        ...
        print("[CARD DEBUG] Error parsing numeric parameters for individual trade card", flush=True)
        return jsonify({"error": "Invalid numeric parameter"}), 400

    if card_type == "stats":
        tab = request.args.get("tab", "crypto") # crypto, stock, free
        print(f"[CARD DEBUG] Entering stats card path. tab={tab}", flush=True)
        ...
        if tab == "free":
            ...
            if tab == "crypto":
                ...
                print(f"[CARD DEBUG] Crypto parameters parsed: overall={param_overall}, daily={param_daily}, wr={param_win_rate}, total={param_total}", flush=True)
                
                if all(v is not None for v in [param_overall, param_daily, param_win_rate, param_total]):
                    try:
                        print("[CARD DEBUG] Parameters complete. Generating card immediately.", flush=True)
                        card_path = media_gen.generate_stats_card(
                            float(param_overall), float(param_daily), float(param_win_rate), int(param_total),
                            user_id=str(ref_id), ref_link=ref_link
                        )
                        if card_path and os.path.exists(card_path):
                            print(f"[CARD DEBUG] Card generated and saved to {card_path}. Sending file.", flush=True)
                            return send_file(card_path, mimetype="image/jpeg", as_attachment=False)
                        else:
                            print("[CARD DEBUG] Failed to generate card path.", flush=True)
                    except Exception as pe:
                        print(f"[SHARE] Error generating card with parameters: {pe}", flush=True)
                
                # Fallback to live fetching
                print("[CARD DEBUG] Fallback: Missing parameters or generation failed. Fetching database and exchange stats.", flush=True)
                ...
                if crypto_api_key and crypto_api_secret:
                    try:
                        print(f"[CARD DEBUG] Connecting to exchange {crypto_exchange_id}...", flush=True)
                        import ccxt
                        config = {
                            "apiKey": crypto_api_key,
                            "secret": crypto_api_secret,
                        ...
                        client = getattr(ccxt, crypto_exchange_id)(config)
                        try:
                            print("[CARD DEBUG] Querying fetch_positions() from exchange...", flush=True)
                            positions = client.fetch_positions()
                            print(f"[CARD DEBUG] Positions fetched: {len(positions)} total objects.", flush=True)
                            for p in positions:
                                contracts = float(p.get("contracts", 0) or 0)
                                if contracts != 0:
                                    crypto_unrealized += float(p.get("unrealizedPnl", 0) or 0)
                ...
                print(f"[CARD DEBUG] Generating stats card for stats: overall_pct={overall_pnl_pct}, daily_pct={daily_pnl_pct}, wr={crypto_win_rate}, total={crypto_total}", flush=True)
                card_path = media_gen.generate_stats_card(
