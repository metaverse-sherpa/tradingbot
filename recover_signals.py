...
async def main():
    SYMBOLS = ['GOOGL', 'AMZN', 'QCOM', 'MU', 'MS', 'CAT', 'ANET', 'AAPL', 'GS']
    ...
    start_time = (datetime.datetime.now() - datetime.timedelta(days=30)).isoformat() + "Z"
    orders = await database.make_alpaca_request_async(user, "GET", f"/v2/orders?status=all&limit=500&after={start_time}")
    ...
    with database.db_session() as conn:
        ...
        for sym in SYMBOLS:
            ...
            # We want the most recent buy/sell pair
            buy_order = None
            sell_order = None
