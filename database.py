...
def get_exchange_client(user, is_async=True):
    ...
    if ex_id == 'alpaca':
        ex_id = 'blofin'
    config = {
        "apiKey": user.get("api_key") or "",
        "secret": user.get("api_secret") or "",
        "password": user.get("api_password") or "NOT_SET",
        "options": {
            "defaultType": "swap",
...
def get_user(chat_id):
    ...
    if row:
        ...
        return {
            ...
            "risk_pct": row_dict.get('risk_pct') if row_dict.get('risk_pct') is not None else 2.0,
...
def get_user_from_web_row(row):
    ...
    return {
        ...
        "risk_pct": row.get('risk_pct') if row.get('risk_pct') is not None else 2.0,
