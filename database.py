        ...
        if hasattr(exchange, 'id') and exchange.id == 'coinbase':
            ...
            try:
                ...
                for exch_sym, grp in fills_by_sym.items():
                    ...
                    contract_size = 1.0
                    try:
                        market = exchange.market(exch_sym)
...
async def update_user_stats_from_engine(chat_id, equity, exchange, application, web_user_id=None):
    ...
    try:
        ...
        if getattr(exchange, 'id', '') == 'coinbase':
            try:
                import logging
                logger = logging.getLogger(__name__)
                # Coinbase specific matching
                all_fills = await exchange.fetch_my_trades(None, since=last_ts, limit=500)
                base_to_sym = {sym.split('/')[0].upper(): sym for sym in symbols_to_check}
                fills_by_sym = {}
                for fill in all_fills:
                        ...
                        if side == 'buy':
                            while qty > 0 and sell_stack:
                                ...
                                if net_pnl > 0:
                                    wins += 1
                                    header = "🏆 *Trade Won!*"
                                    share_data = f"sh_{canonical_sym}_s_{roe_pct:.2f}_{opener['price']}_{price}_{net_pnl:.2f}"
                                else:
                                    losses += 1
                                    header = "❌ *Trade Lost*"
                                    share_data = None
                            ...
                            while qty > 0 and buy_stack:
                                ...
                                if net_pnl > 0:
                                    wins += 1
                                    header = "🏆 *Trade Won!*"
                                    share_data = f"sh_{canonical_sym}_l_{roe_pct:.2f}_{opener['price']}_{price}_{net_pnl:.2f}"
                                else:
                                    losses += 1
                                    header = "❌ *Trade Lost*"
                                    share_data = None
                ...
                import logging
                logging.getLogger(__name__).error(f"Error fetching Coinbase history: {e}")
        else:
...
def add_premium_days(chat_id, days):
    ...
    with db_session() as conn:
        c = conn.cursor()
        c.execute("UPDATE Users SET premium_expiry = ?, premium_expired_notified = FALSE, premium_warning_notified = FALSE, had_premium_before = TRUE WHERE telegram_chat_id = ?", (new_expiry, chat_id))

def revoke_premium(chat_id):
    """Revokes a user's premium status immediately."""
    with db_session() as conn:
        c = conn.cursor()
        c.execute("UPDATE Users SET premium_expiry = 0, premium_expired_notified = TRUE, premium_warning_notified = TRUE WHERE telegram_chat_id = ?", (chat_id,))
...
def get_expired_unnotified_users():
    ...
    with db_session() as conn:
        c = conn.cursor()
        c.execute("SELECT telegram_chat_id FROM Users WHERE premium_expiry > 0 AND premium_expiry < ? AND premium_expired_notified = FALSE", (now,))
...
def get_expired_unnotified_web_users(now_ts):
    ...
    with db_session() as conn:
        ...
        c.execute('''
            SELECT id, telegram_chat_id, email, full_name, premium_expiry 
            FROM WebUsers 
            WHERE premium_expiry > 0 AND premium_expiry < ? AND (premium_expired_notified = FALSE OR premium_expired_notified IS NULL)
        ''', (now_ts,))
        return [dict(row) for row in c.fetchall()]
...
def redeem_gift_code(chat_id, code, current_username=None):
    ...
    with db_session() as conn:
        ...
        c.execute('UPDATE Users SET premium_expiry = ?, premium_expired_notified = FALSE, premium_warning_notified = FALSE WHERE telegram_chat_id = ?', (new_expiry, chat_id))
...
def redeem_gift_code_web(web_user_id, code):
    ...
    with db_session() as conn:
        ...
        if telegram_chat_id:
            ...
            c.execute('UPDATE Users SET premium_expiry = ?, premium_expired_notified = FALSE, premium_warning_notified = FALSE WHERE telegram_chat_id = ?', (new_bot_expiry, telegram_chat_id))
